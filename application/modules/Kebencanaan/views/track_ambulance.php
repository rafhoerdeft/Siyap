...
<script>  
    ...
    function initialize_map() {
      ...
      var database = firebase.database().ref().child('/Ambulance');
        ...
        if (feature) {
          if (feature.get('role')=='User') {
            ...
            var contents = '<p>ID Aduan: <b>'+feature.get('id_lapor')+'</b></p>'+
                           ...
                           "<a class='foto_kejadian' href=<?= base_url().'assets/path_selfie/' ?>"+feature.get('img_selfie')+" rel='selfie'> <button class='btn btn-block bg-pink waves-effect'>FOTO SELFIE</button> </a>"+
                           "</div>"+
                           "</center></p>";
...
<script type="text/javascript">
  function lapSelesai(id, key) {
      ...
      swal({
          ...
          setTimeout(function () {
              // var remove = firebase.database().ref('Ambulance/'+key).remove();
              var remove = 0;
              var ref = firebase.database().ref().child('/Ambulance');
              ...
              if (remove > 0) {
                  $.post("<?= base_url() ?>/Ambulance/lapSelesai", {id_lap: id, key: key}, function(result){
                    // alert(result);
                      if (result == 'Success') {
