                                ...
                                <tbody>
                                    <?php $no=1; foreach ($user as $data) { ?>
                                        <tr>
                                            ...
                                            <td nowrap>
                                                <button title="Edit" class="btn btn-sm btn-sm btn-raised bg-green waves-effect" data-toggle="modal" onclick="editModal('<?= $data->id_user ?>','<?= $data->nama ?>','<?= $data->jenis_kelamin ?>','<?= $data->alamat ?>','<?= date('d-m-Y', strtotime($data->tgl_lahir)) ?>','<?= $data->no_hp ?>','<?= $data->password ?>','<?= str_replace(" ", "_", $data->role) ?>','<?= $data->username ?>','<?= $data->id_wmk ?>','<?= $data->id_regu ?>','<?= $data->jabatan ?>')" style="margin-bottom: 5px; width: 5px;"><i class="material-icons" style="font-size: 12pt; margin-top: -5px; margin-left: -5px;">border_color</i></button>
...
<script type="text/javascript">
    ...
    function editModal(id='', nama='', jk_user='', almt='', tgl_lhr='', no_hp='', pass='', role='', user='', wmk='', regu='', jabatan='') {
        // alert(jk_user);
        clear_data();
